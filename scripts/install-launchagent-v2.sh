...
LAUNCH_LABEL="${TICKET_LAUNCHAGENT_LABEL:-ai.openclaw.ticket-platform-v2-api}"
PLIST_PATH="${TICKET_LAUNCHAGENT_PLIST_PATH:-$HOME/Library/LaunchAgents/${LAUNCH_LABEL}.plist}"
LOG_DIR="${TICKET_API_LOG_DIR:-$REPO_DIR/logs/v2}"
RUN_API_SCRIPT="${TICKET_RUN_API_SCRIPT:-$REPO_DIR/scripts/run-api-v2.sh}"
...
<dict>
  ...
  <dict>
    ...
    <string>${TICKETS_DB_PATH:-$REPO_DIR/data/ticket-platform-v2.db}</string>
    <key>TICKET_INTERNAL_POLLERS_ENABLED</key>
    <string>${TICKET_INTERNAL_POLLERS_ENABLED:-false}</string>
    ...
    <string>${TICKET_API_PORT:-8790}</string>
    <key>TICKET_API_BIND_HOST</key>
    <string>${TICKET_API_BIND_HOST:-127.0.0.1}</string>
    <key>TICKET_API_LOCAL_BASE_URL</key>
    <string>${TICKET_API_LOCAL_BASE_URL:-http://127.0.0.1:${TICKET_API_PORT:-8789}}</string>
    <key>TICKET_API_HEALTHCHECK_BASE_URL</key>
    <string>${TICKET_API_HEALTHCHECK_BASE_URL:-http://127.0.0.1:${TICKET_API_PORT:-8789}}</string>
    <key>TICKET_BACKUP_BEFORE_START</key>
    <string>${TICKET_BACKUP_BEFORE_START:-false}</string>
    <key>NODE_BIN</key>
